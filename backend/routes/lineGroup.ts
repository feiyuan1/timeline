...
const routeGroup = (router: Router<any, Koa.BeContext<types.LineGroup>>) => {
  router.use(collectionMiddleware<types.LineGroup>('lineGroup'))
  ...
  router.post(`${prefix}/:id`, async (ctx, next) => {
    ...
    const data = request.body as types.LineGroup
  ...
  router.delete(`${prefix}/:id`, async (ctx, next) => {
    ...
    const { lines } = await collection.findOne({ id })
    if (lines.length) {
      if (query.deleteLine) {
        // delete line
      } else {
  ...
  router.use(prefix, responseMiddleware<types.LineGroup>())
}
